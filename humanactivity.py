...
net = cv2.dnn.readNet(args['model']) #use the dnn model to read pre-trained human activity 

# grab a pointer to the input video stream
print('[INFO] accessing video stream…')
vs = cv2.VideoCapture(args['input'] if args['input'] else 0) #instantiate a video file in input or a webcam
...
while True:
 ...
 for i in range(0, SAMPLE_DURATION):
    (grabbed, frame) = vs.read() # read a frame from the video stream
